			...
			<table class="table table-bordered">
				...
				<tbody>
					@foreach($bonds AS $bond)
						<tr>
							<td>{{ $bond->id }}</td>
							<td><img src="/assets/images/npcs/images/{{ $bond->image }}" /></td>
							<td colspan="2">{{ $bond->description }}</td>
							<td>{{ $bond->bond_type }}</td>
							<td>
								<a href="/admin/npcs/bond-edit/{{ $bond->id }}" class="btn btn-warning">Edit</a>
								<a href="/admin/npcs/bond-delete/{{ $bond->id }}" class="btn btn-danger">Delete</a>
							</td>
						</tr>
					@endforeach
				</tbody>
