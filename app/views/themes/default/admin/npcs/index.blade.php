			...
			<table class="table table-bordered">
				...
				<tbody>
					@foreach($npcs AS $npc)
						<tr>
							<td>{{ $npc->id }}</td>
							<td><img src="/assets/images/npcs/icons/{{ $npc->icon }}" /></td>
							<td>{{ $npc->name }}</td>
							<td colspan="2">
							    {{ $npc->description }}
							</td>
							<td>{{ ($npc->bond) ? 'Yes' : 'No' }}</td>
							<td>
								<a href="/admin/npcs/edit/{{ $npc->id }}" class="btn btn-warning">Edit</a>
								<a href="/admin/npcs/delete/{{ $npc->id }}" class="btn btn-danger">Delete</a>
							</td>
						</tr>
					@endforeach
				</tbody>
