...
@section('content')
	...
	<div style="text-align: right; margin-top: 20px;">
		<a href="/admin/promo-codes/new" class="btn btn-success">Add Promo</a>
	</div>
				...
				<tbody>
					@foreach($promos AS $promo)
						<tr>
							<td>{{ $promo->id }}</td>
							<td>{{ $promo->code }}</td>
							<td>{{ ucfirst($promo->species->name) }}</td>

							<td>
								<a href="/admin/promo-codes/edit/{{ $promo->id }}" class="btn btn-warning">Edit</a>
								<a href="/admin/promo-codes/delete/{{ $promo->id }}" class="btn btn-danger">Delete</a>
							</td>
						</tr>
					@endforeach
				</tbody>
