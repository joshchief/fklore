...
@section('content')
	...
	<div style="text-align: right; margin-top: 20px;">
		...
		<a href="/admin/shops/greetings/{{ $shop->id }}" class="btn btn-success">Greetings</a>
		<a href="/admin/shops/items/{{ $shop->id }}" class="btn btn-success">Items</a>
	</div>

	<div class="row">
		<div class="col-md-12" style="margin-top: 20px; text-align: center;">
			{{ Form::model($shop, array('url' => '/admin/shops/edit/'.$shop->id, 'method' => 'post', 'files' => true)) }}
			
			    <p>
			         <center><img src="/assets/images/shops/{{ $shop->image }}?time()" /></center>
			    </p>
				...
				<p>
				    ...
				    <select name="npc_id">
				        @foreach($npcs AS $npc)
				            <option value="{{ $npc->id }}" @if($npc->id == $shop->npc_id) selected @endif>{{ ucfirst($npc->name) }}</option>
				        @endforeach
				    </select>
				</p>
				...
				<p>
					...
					{{ Form::checkbox('mon', 1, $shop->mon) }} Mon
					{{ Form::checkbox('tue', 1, $shop->tue) }} Tue
					{{ Form::checkbox('wed', 1, $shop->wed) }} Wed
					{{ Form::checkbox('thu', 1, $shop->thu) }} Thu
					{{ Form::checkbox('fri', 1, $shop->fri) }} Fri
					{{ Form::checkbox('sat', 1, $shop->sat) }} Sat
					{{ Form::checkbox('sun', 1, $shop->sun) }} Sun
				</p>
