...
@section('content')
	...
	<div style="text-align: right; margin-top: 20px;">
		<a href="/admin/shops/greetings-add/{{ $greeting->id }}" class="btn btn-success">New</a>
	</div>
	...
	<div class="row">
		<div class="col-md-12" style="margin-top: 20px; text-align: center;">
			{{ Form::model($greeting) }}
				<p>
				    {{ Form::label('greeting', 'Greeting:') }}<br />
				    <textarea name="greeting" cols="80" rows="10">{{ $greeting->greeting }}</textarea>
				</p>
				
				<p>
				    {{ Form::label('friendship', 'Friendship:') }}
				    <select name="friendship">
				        <option value="0"  @if(!$greeting->friendship) selected @endif>No</option>
				        <option value="1" @if($greeting->friendship) selected @endif>Yes</option>
				    </select>
				</p>
