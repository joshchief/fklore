...
@section('content')
	...
	<div style="text-align: right; margin-top: 20px;">
		<a href="/admin/shops/greetings-add/{{ $shop->id }}" class="btn btn-success">New</a>
	</div>
			...
			<table class="table table-bordered">
				<thead>
					<tr>
						...
						<th style="width: 10%;">Greeting</th>
						<th style="width: 10%;">Friendship</th>
						<th style="width: 10%;">Options</th>
					</tr>
				</thead>

				<tbody>
					@foreach($shop->greetings AS $greeting)
						<tr>
							<td>{{ $greeting->id }}</td>
							<td>{{ $shop->name }}</td>
							<td>{{ $greeting->greeting }}</td>
							<td>{{ ($greeting->friendship) ? 'Yes' : 'No' }}</td>
							<td>
								<a href="/admin/shops/greetings-edit/{{ $greeting->id }}" class="btn btn-warning">Edit</a>
								<a href="/admin/shops/greetings-delete/{{ $greeting->id }}" class="btn btn-danger">Delete</a>
							</td>
						</tr>
					@endforeach
				</tbody>
