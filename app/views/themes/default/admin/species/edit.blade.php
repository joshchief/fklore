...
@section('content')
				...
				<p>
				    ...
				    <select name="bloodline_id">
				        @foreach($bloodlines AS $line)
				            <option value="{{ $line->id }}" @if($line->id == $species->bloodline_id) selected @endif>{{ ucfirst($line->name) }}</option>
				        @endforeach
				    </select>
				</p>
 ...
 @if($species->horns) checked="checked" @endif> Horns?
				</p>
                
                <p>
				    <input type="checkbox" name="horns_pointed" value="1"
 @if($species->horns_pointed) checked="checked" @endif> Pointed?
				</p>
				
				<p>
				    <input type="checkbox" name="promo" value="1"
 @if($species->promo) checked="checked" @endif> Promo?
				</p>
